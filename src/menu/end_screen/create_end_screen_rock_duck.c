...
int update_end_screen_rock_duck(game_t *game, projectile_t *coll)
{
    coll->pos.x -= coll->spd.x - (cosf(coll->lifetime++ / 50.0) * 10);
    coll->pos.y -= coll->spd.y - (sinf(coll->lifetime++ / 50.0) * 10);
    add_node(create_end_screen_star_particle(game, coll->pos, ROCK_COLOR),
            &(game->particles));
    return 0;
}
...
projectile_t *create_end_screen_rock_duck(game_t *game)
{
    ...
    dest->color = sfWhite;
    dest->type = ROCK_DUCK;
    dest->update = update_end_screen_rock_duck;
    dest->rot = 0;
